...
class RemoveContentFromHiddenParent {
	...
	public function contentElementShouldBeIndexed(array $ttContent, bool $isIndexed, Page $pageIndexer) {
		...
		// keine verschachtelten Inhaltselemente von xo-elements -> darf indexiert werden
		if(empty($ttContent['tx_xo_parent']) === true || (int) $ttContent['colPos'] !== 5628) {
			return $isIndexed;
		}
		...
		$statement = $queryBuilder
			...
			->where(
				$queryBuilder->expr()->eq('uid', $queryBuilder->createNamedParameter((int) $ttContent['tx_xo_parent'], Connection::PARAM_INT)),
		...
		// ---
		// 2. Pruefung ob das Elternelement von tx_xo_domain_model_elements aktiv ist
